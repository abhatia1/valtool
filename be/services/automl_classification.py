"""
AutoML Classification Training Engine

Comprehensive classification training with preprocessing, feature engineering,
...
from sklearn.model_selection import (
    cross_validate, KFold, StratifiedKFold, GridSearchCV,
...
# ============================================================
# Valid Scoring Metrics for Classification
# ============================================================

VALID_CLASSIFICATION_METRICS = {
    'accuracy': {
        'name': 'Accuracy',
        'sklearn_name': 'accuracy',
        'description': 'Accuracy score (higher is better)',
        'higher_is_better': True,
        'metric_key': 'accuracy'  # Key in _calculate_metrics output
    },
    'f1': {
        'name': 'F1 Score (Weighted)',
        'sklearn_name': 'f1_weighted',
        'description': 'F1 score with weighted averaging (higher is better)',
        'higher_is_better': True,
        'metric_key': 'f1_weighted'
    },
    'f1_macro': {
        'name': 'F1 Score (Macro)',
        'sklearn_name': 'f1_macro',
        'description': 'F1 score with macro averaging (higher is better)',
        'higher_is_better': True,
        'metric_key': 'f1_macro'
    },
    'f1_micro': {
        'name': 'F1 Score (Micro)',
        'sklearn_name': 'f1_micro',
        'description': 'F1 score with micro averaging (higher is better)',
        'higher_is_better': True,
        'metric_key': 'f1_micro'
    },
    'precision': {
        'name': 'Precision (Weighted)',
        'sklearn_name': 'precision_weighted',
        'description': 'Precision with weighted averaging (higher is better)',
        'higher_is_better': True,
        'metric_key': 'precision_weighted'
    },
    'precision_macro': {
        'name': 'Precision (Macro)',
        'sklearn_name': 'precision_macro',
        'description': 'Precision with macro averaging (higher is better)',
        'higher_is_better': True,
        'metric_key': 'precision_macro'
    },
    'recall': {
        'name': 'Recall (Weighted)',
        'sklearn_name': 'recall_weighted',
        'description': 'Recall with weighted averaging (higher is better)',
        'higher_is_better': True,
        'metric_key': 'recall_weighted'
    },
    'recall_macro': {
        'name': 'Recall (Macro)',
        'sklearn_name': 'recall_macro',
        'description': 'Recall with macro averaging (higher is better)',
        'higher_is_better': True,
        'metric_key': 'recall_macro'
    },
    'roc_auc': {
        'name': 'ROC AUC',
        'sklearn_name': 'roc_auc_ovr',  # Will use appropriate variant based on binary/multiclass
        'description': 'ROC AUC score (higher is better)',
        'higher_is_better': True,
        'metric_key': 'roc_auc'
    },
    'cohen_kappa': {
        'name': 'Cohen Kappa',
        'sklearn_name': None,  # Not a standard sklearn scorer
        'description': 'Cohen kappa score (higher is better)',
        'higher_is_better': True,
...
if XGBOOST_AVAILABLE:
    CLASSIFICATION_ESTIMATORS['xgboost'] = {
        'class': XGBClassifier,
        'default_params': {
            'random_state': 42,
            'n_jobs': -1,
...
class ClassificationTrainer:
    """
    Comprehensive AutoML classification trainer.
    ...
    def _handle_outliers(self, X: pd.DataFrame, y: np.ndarray, method: str) -> Tuple[pd.DataFrame, np.ndarray]:
        """
        Handle outliers using specified method.
        
        FIX: Now returns (X, y) tuple to handle row removal consistently.
        """
        ...
        if method == 'iqr':
            ...
            print(f"Clipped outliers in {len(numeric_cols)} numeric features")
            ...
            outliers = iso_forest.fit_predict(X[numeric_cols].fillna(0))
            # Keep inliers only (outliers == 1 means inlier in sklearn)
            mask = outliers == 1
            X_clean = X_clean[mask].reset_index(drop=True)
            y_clean = y_clean[mask]
    ...
    def engineer_features(self) -> None:
        ...
        if feat_config.get('feature_selection', False):
            ...
            if method == 'selectkbest':
                self.feature_selector = SelectKBest(f_classif, k=min(n_features, self.X_processed.shape[1]))
            elif method == 'mutual_info':
                self.feature_selector = SelectKBest(mutual_info_classif, k=min(n_features, self.X_processed.shape[1]))
            else:
                self.feature_selector = SelectKBest(f_classif, k=min(n_features, self.X_processed.shape[1]))
    ...
    def train_models(self) -> None:
        ...
        scoring_metric = model_config.get('scoring_metric', DEFAULT_CLASSIFICATION_METRIC)

        # Validate scoring metric
        if scoring_metric not in VALID_CLASSIFICATION_METRICS:
            print(f"⚠ Warning: Invalid scoring metric '{scoring_metric}' for classification.")
            print(f"  Valid options: {', '.join(VALID_CLASSIFICATION_METRICS.keys())}")
            print(f"  Defaulting to '{DEFAULT_CLASSIFICATION_METRIC}'")
            scoring_metric = DEFAULT_CLASSIFICATION_METRIC

        metric_info = VALID_CLASSIFICATION_METRICS[scoring_metric]
        ...
        for i, est_name in enumerate(estimator_names, 1):
            ...
            if est_name not in CLASSIFICATION_ESTIMATORS:
        ...
        # Sort by score (higher is better for all our metrics)
        ...
        # Show accuracy for reference if it's not the primary metric
        if scoring_metric != 'accuracy':
            acc_key = 'train_accuracy' if 'train_accuracy' in self.results[self.best_model_name]['metrics'] else None
            if acc_key:
                print(f"   Accuracy: {self.results[self.best_model_name]['metrics'][acc_key]:.4f}")

    def _init_mlflow(self, mlflow_config: Dict) -> None:
        """Initialize MLflow tracking"""
        experiment_name = mlflow_config.get('experiment_name', 'automl_classification')
    ...
    def _train_single_model(
        ...
        """
        est_info = CLASSIFICATION_ESTIMATORS[est_name]
        ...
        # Validate scoring metric
        if scoring_metric not in VALID_CLASSIFICATION_METRICS:
            print(f"  ⚠ Invalid scoring metric '{scoring_metric}', using default 'accuracy'")
            scoring_metric = DEFAULT_CLASSIFICATION_METRIC
        
        sklearn_scoring = VALID_CLASSIFICATION_METRICS[scoring_metric]['sklearn_name']
        metric_key = VALID_CLASSIFICATION_METRICS[scoring_metric]['metric_key']
        ...
        if validation_strategy == "train_test_split":
            ...
            X_train_inner, X_val_inner, y_train_inner, y_val_inner = train_test_split(
                ...
                test_size=test_size,
                random_state=42,
                stratify=self.y_processed  # Stratified split for classification
            )
            ...
            y_pred = best_model.predict(X_val_inner)
            y_pred_proba = None
            if hasattr(best_model, 'predict_proba'):
                try:
                    y_pred_proba = best_model.predict_proba(X_val_inner).tolist()
                except Exception:
                    pass  # Some models may not support predict_proba

            predictions = {
                'y_true': y_val_inner.tolist(),
                'y_pred': y_pred.tolist(),
                'y_pred_proba': y_pred_proba,
                'class_names': [str(c) for c in self.label_encoder.classes_] if self.label_encoder else None
            }
            ...
            # Store predictions for visualization (on full training data for CV)
            y_pred = best_model.predict(self.X_processed)
            y_pred_proba = None
            if hasattr(best_model, 'predict_proba'):
                try:
                    y_pred_proba = best_model.predict_proba(self.X_processed).tolist()
                except Exception:
                    pass  # Some models may not support predict_proba

            predictions = {
                'y_true': self.y_processed.tolist(),
                'y_pred': y_pred.tolist(),
                'y_pred_proba': y_pred_proba,
                'class_names': [str(c) for c in self.label_encoder.classes_] if self.label_encoder else None
            }
    ...
    def _calculate_metrics(self, model, X, y) -> Dict[str, float]:
        ...
        # Add macro/micro averages for multiclass
        if n_classes > 2:
            metrics.update({
    ...
    def evaluate_best_model(self) -> Dict[str, Any]:
        ...
        NEW: Stores predictions (y_true, y_pred, y_pred_proba) for visualizations.
        """
        ...
        test_metrics = self._calculate_metrics(self.best_model, X_test_processed, self.y_test)
        ...
        if hasattr(self.best_model, 'predict_proba'):
            y_pred_proba = self.best_model.predict_proba(X_test_processed).tolist()
        elif hasattr(self.best_model, 'decision_function'):
            # For models with decision_function (like SVC), use that
            decision_scores = self.best_model.decision_function(X_test_processed)
            # Convert to probability-like scores using sigmoid
            from scipy.special import expit
            if len(decision_scores.shape) == 1:
                # Binary classification
                proba_pos = expit(decision_scores)
                y_pred_proba = [[1 - p, p] for p in proba_pos]
            else:
                # Multi-class
                y_pred_proba = expit(decision_scores).tolist()
        ...
        return {
            ...
            # NEW: Store predictions for visualizations
            'predictions': {
